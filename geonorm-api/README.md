...
```bash
geonorm-api$ sam build --use-container
```
...
```bash
geonorm-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
geonorm-api$ sam local start-api
geonorm-api$ curl http://localhost:3000/
```
...
```bash
geonorm-api$ sam logs -n HelloWorldFunction --stack-name geonorm-api --tail
```
...
```bash
geonorm-api$ pip install -r tests/requirements.txt --user
# unit test
geonorm-api$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
geonorm-api$ AWS_SAM_STACK_NAME="geonorm-api" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "geonorm-api"
```
